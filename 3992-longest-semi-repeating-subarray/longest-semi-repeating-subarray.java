class Solution {
    public int longestSubarray(int[] nums, int k) {
        ...
        for(;right<n; right++){
            if(hm1.containsKey(nums[right])) hm2.put(nums[right], hm2.getOrDefault(nums[right],0)+1);
            ...
            while(left<right && hm2.size()>k){
                ...
                if(hm1.get(nums[left])==0) hm1.remove(nums[left]);
                hm2.put(nums[left],hm2.getOrDefault(nums[left],0)-1);
                if(hm2.get(nums[left])<=0) hm2.remove(nums[left]);
                left++;
            }
