...
ACTIVE_USERS AS (SELECT se.user_id, se.plan_name as current_plan , se.monthly_amount as current_monthly_amount, le.max_historical_amount, sf60.days_as_subscriber  FROM subscription_events se right join LAST_EVENT le on le.user_id = se.user_id and le.last_event = se.event_id 
